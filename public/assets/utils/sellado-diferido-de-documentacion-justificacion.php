<?php	
		...
		$doc->base64 = chunk_split(base64_encode(file_get_contents(WRITEPATH.'documentos/'.$nif.'/justificacion/'.$selloDeTiempo.'/'.$nombreDelArchivo)));
	...
	function printResult($result, $idDoc, $nombreDelArchivo) {
		...
		if ($respuesta['publicAccessId']) {
			...
			$sql = "UPDATE pindust_documentos_justificacion SET custodiado = 1, fechaCustodiado = CURDATE(), publicAccessIdCustodiado ='".$respuesta['publicAccessId']."' WHERE id=".$idDoc."; ";
