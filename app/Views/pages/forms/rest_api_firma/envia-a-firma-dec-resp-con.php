<?php
		...
		$adreca_mail = $_POST["adreca_mail_representante"];
		...
		$user->action = "SIGN";  
		$user->userCode = $email_rep; //"ignacio.llado@idi.es";    
		$user->userName = $empresa; //"wwwwwwwwwwwwwwww";
		$user->userSurname1 = $nif; //"43036826P";
		$user->userSurname2 = "";
		$user->userPhone = "+34".$telefono_rep; //"+34677234076";
		...
		$request->subject = lang('message_lang.dec_resp_con_menu'); // "iDigital: Declaració responsable del consultor";
		...
		$doc->filename = $nif.'_dec_res_consultor_iDigital.pdf';
		$doc->base64 = chunk_split(base64_encode(file_get_contents(WRITEPATH.'documentos/consultor/'.$nif.'/'.$selloDeTiempo.'/'.$nif.'_dec_res_consultor_iDigital.pdf')));
	...
	function printResult($result, $last_insert_id, $tipo_Doc) {
		...
		// echo "<pre><code>".var_dump ($respuesta)."</code></pre>";
		// echo "<pre><code>".$respuesta['sender']['userCode']."</code></pre>";
		// echo "<pre><code>".lang('message_lang.generado_correctamente')."<b>".$respuesta['subject']."</b></code></pre>";
		// echo "<pre><code>".$respuesta['notificationUrl']."</code></pre>";
		// echo "<pre><code>".$respuesta['publicAccessId']."</code></pre>";
		// echo "<pre><code>Last insert id: ".$last_insert_id."</code></pre>";

		$db = \Config\Database::connect();
		$builder = $db->table('pindust_consultor');
		$data = [
		...
		echo "-<pre><code>".lang('message_lang.enviado_correo_electron_con_dec_res')."</code></pre>";
		echo "---<pre><code>".lang('message_lang.una_vez_firmado')."</code></pre>";
		echo '-</div>';
	}
?>