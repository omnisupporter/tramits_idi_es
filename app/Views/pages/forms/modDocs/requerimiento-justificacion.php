...
<div class="card-itramits">
	...
	<div class="card-itramits-footer">
        ...
        <?php }
			...
			<button type = "button" class = "btn-primary-itramits" data-bs-toggle="modal" data-bs-target="#myRequerimientoJustificacion" id="myBtnRequerimientoJustificacion">Motiu del requeriment</button>
...
<!--     			<a id="wrapper_generadoc_req_justificacion" class="ocultar" href="<?php echo base_url('public/index.php/expedientes/generaInforme/'.$id.'/'.$convocatoria.'/'.$programa.'/'.$nifcif.'/doc_requerimiento_justificacion');?>" class="btn-primary-itramits">Envia a la firma de Gerència IDI</a> -->
	...
	<div class="card-itramits-footer">
		<?php if ($expedientes['doc_requerimiento_justificacion'] !=0) { ?>
			...
			<!-- <a	class='btn btn-ver-itramits' href="<?php echo base_url('public/index.php/expedientes/muestrainforme/'.$id.'/'.$convocatoria.'/'.$programa.'/'.$nifcif.'/doc_requerimiento_justificacion');?>" target = "_self"><i class='fa fa-check'></i>El requeriment d’esmena</a>		 -->

			<?php 
/* 			$db = \Config\Database::connect();
			$sql = "SELECT * FROM pindust_documentos_generados WHERE name='doc_requerimiento_justificacion.pdf' AND id_sol=".$expedientes['id']." AND convocatoria='".$expedientes['convocatoria']."'";// AND tipo_tramite='".$expedientes['tipo_tramite']."'";
			$query = $db->query($sql);
			$row = $query->getRow(); */
