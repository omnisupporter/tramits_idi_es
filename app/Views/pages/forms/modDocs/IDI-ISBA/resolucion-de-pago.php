...
<div class="card-itramits">
	...
	<div class="card-itramits-footer">
		<?php if ($expedientes['doc_res_pago_con_req'] != 0) { ?>
			<?php
			$tieneDocumentosGenerados = $modelDocumentosGenerados->documentosGeneradosPorExpedYTipo($expedientes['id'], $expedientes['convocatoria'], 'doc_res_pago_con_req_idi_isba.pdf');
...
<script>
	function enviaResolucionPagoConReg(id, convocatoria, programa, nifcif) {
		let todoBien = true
		...
		if (todoBien) {
			...
			window.location.href = base_url + '/' + id + '/' + convocatoria + '/' + programa + '/' + nifcif + '/doc_res_pago_idi_isba'
		} else {
