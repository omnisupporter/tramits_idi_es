...
<article>
  ...
  <div class="alert alert-info">
		<h3><strong><?php echo lang('message_lang.solicitud_doc_informe_resumen_ils_titulo');?></strong></h3>
  </div>

  <form action="<?php echo base_url('/public/index.php/expedientes/do_doc_informe_resumen_upload/'.$data['expedientes']['id'].'/'.$data['expedientes']['nif'].'/'.$data['expedientes']['tipo_tramite'].'/'.$data['expedientes']['convocatoria'].'/'. $idioma);?>" name="form_informeGEH" id="form_informeGEH" method="post" accept-charset="utf-8" enctype="multipart/form-data">
  ...
  <fieldset> 
    <div>
      <h3><strong><?php echo $data['expedientes']['empresa'];?></strong></h3><br>					
          ...
          <?php 
	          echo lang('message_lang.informe_resumen');
          ?>
        </h3>
    </div>
  </fieldset> 
  ...
  <fieldset>  
  	<legend><h4><strong><?php echo lang('message_lang.documento_informe_resumen_ils');?></strong></h4> </legend>
	  <section class="panel-justificacion">
		  <div class = "content-file-upload">
  			<h5>[.pdf, .jpg, .png, .webp] <span class="container-radio-invalid">(Max. file size: 10.0 M)</span>:</h5>
	  		  <div>
					  <input type="file" onchange="detectExtendedASCII(this.id, this.files)" id = "file_informeResumenIls" name="file_informeResumenIls[]" required size="20" accept=".pdf, .jpg, .png, .webp"/>
				  </div>
		  </div>
	  </section>
	</fieldset>
